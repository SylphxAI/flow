...
- A preview deployment must pass before merge when the repository has a configured preview deployment provider.
- Production deployment happens only after merge through the documented release path.

Do not merge while a required check is pending, skipped unexpectedly, missing, or failing. Do not merge while a configured required preview deployment is missing or failing. Treat configured preview deployment as executable acceptance evidence, not as a nice-to-have link.

If a required check or configured preview deployment fails, fix the root cause when it is in scope. If the failure is caused by unavailable credentials, blocked billing, protected environment approval, provider outage, or another external condition, stop at the narrow blocker and report the exact blocked gate instead of bypassing protection.

Do not hardcode a hosting vendor into generic agent instructions. A real preview deployment may be provided by Vercel, Cloudflare Pages, Netlify, GitHub Pages, Kubernetes review apps, or another documented platform. Do not substitute a local build, artifact upload, or fake status check for a preview deployment.
